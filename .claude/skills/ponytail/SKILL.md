...
No drift back to over-building.
...
It runs *after* you understand the problem, not instead of it.
...
- **lite:** Build what was requested and name the simpler alternative.
  The user chooses.
- **full:** Enforce the ladder, using the standard library and native options first.
  Keep the diff and explanation short.
  This is the default.
- **ultra:** Question unnecessary features before adding code.
  Prefer deletion and the smallest correct solution.

Check out these **examples** for "Add a cache for these API responses":
...
Lazy code without regression coverage is unfinished.
Non-trivial logic leaves a focused check that fails if the behavior breaks.
Use the repository's test framework and run `task lint` and `task test` locally before every commit.
Trivial prose changes need no test, and YAGNI applies to tests too.
