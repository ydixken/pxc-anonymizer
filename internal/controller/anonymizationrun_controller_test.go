...
var _ = Describe("AnonymizationRun Controller", func() {
	Context("When reconciling a resource", func() {
		...
		anonymizationrun := &pxcanonymizeriov1alpha1.AnonymizationRun{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind AnonymizationRun")
			err := k8sClient.Get(ctx, typeNamespacedName, anonymizationrun)
			if err != nil && errors.IsNotFound(err) {
				resource := &pxcanonymizeriov1alpha1.AnonymizationRun{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance AnonymizationRun")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &AnonymizationRunReconciler{
				Client: k8sClient,
